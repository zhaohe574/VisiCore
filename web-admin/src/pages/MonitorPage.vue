...
import { Check, Close, DArrowRight, Download, FullScreen, Refresh, RefreshLeft, RefreshRight, Search, Star, StarFilled, VideoPause, VideoPlay } from '@element-plus/icons-vue'
...
import { captureLayout, layoutColumns, layoutOptions, patrolChannels, restoreLayout, type LayoutCount } from '../lib/layouts'
...
const count = ref<LayoutCount>(4), selected = ref(0), slots = ref<(Channel | undefined)[]>(Array(16).fill(undefined)), streams = ref<(1 | 2)[]>(Array(16).fill(2)), slotRanges = ref<({ start: string; end: string } | undefined)[]>(Array(16).fill(undefined)), sessions = ref<(LiveSession | PlaybackSession | null)[]>(Array(16).fill(null)), revision = ref(0)
type TileInstance = InstanceType<typeof VideoTile>
const tiles = new Map<number, TileInstance>(), wall = ref<HTMLElement>(), searchDate = ref<Date>(new Date()), searchStartTime = ref<string>('00:00:00'), recordings = ref<Recording[]>([]), unified = ref(false), speed = ref(1), layoutId = ref<number>(), patrol = ref<Layout>(), patrolOffset = ref(0), layoutDialog = ref(false), layoutName = ref('')
...
function setCount(value: LayoutCount) { stopPatrol(); for (let index = value; index < 16; index++) { slots.value[index] = undefined; slotRanges.value[index] = undefined; sessions.value[index] = null }; count.value = value; selected.value = Math.min(selected.value, value - 1) }
...
async function searchRecordings() {
  ...
  await run(async () => { if (!channel) throw new Error('请选择查询通道'); const time = timeRange(currentRange.value); const found = await mediaApi.recordings(channel.id, time.start, time.end); if (current === searchGeneration && selectedChannel.value?.id === channel.id) recordings.value = found }, '')
}
async function startPlayback(recording?: Recording) {
  await run(async () => { const time = recording ? { start: recording.start, end: recording.end } : timeRange(currentRange.value); const indices = unified.value ? Array.from({ length: count.value }, (_, index) => index).filter(index => !!slots.value[index]) : [selected.value]; if (!indices.some(index => !!slots.value[index])) throw new Error('请选择回放通道'); for (const index of indices) if (slots.value[index]) slotRanges.value[index] = { ...time }; await nextTick() }, '')
}
  ...
  for (let i = 0; i < count.value; i++) {
    ...
    if (ch) {
      result.push({
        ...
        segments: s?.segments || (selected.value === i && recordings.value.length ? recordings.value.map(r => ({ start: r.start, end: r.end })) : []),
...
function setPresetDate(preset: 'today' | 'yesterday') {
  if (preset === 'today') {
...
async function exportSelected() { await run(async () => { if (!selectedChannel.value) throw new Error('请选择导出通道'); const time = timeRange(currentRange.value, 24); await workflowApi.createExport(selectedChannel.value.id, time.start, time.end) }, '导出任务已提交，可在录像导出中查看') }
