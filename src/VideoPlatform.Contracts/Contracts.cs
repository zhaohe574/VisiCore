...
public sealed record DeviceRequest(string Name, string Host, int Port, string Username, string? Password, bool Enabled = true);
public sealed record DeviceDto(long Id, string Name, string Host, int Port, string Username, bool Enabled, string Status, string? Model, string? SerialNumber, DateTimeOffset? LastSeenAt, long ChannelCount, long OnlineChannels);
public sealed record ChannelDto(long Id, long DeviceId, string DeviceName, int DeviceChannel, string Name, string? Model, string Status, long? UnitId, bool PtzCapable, string? Codec);
