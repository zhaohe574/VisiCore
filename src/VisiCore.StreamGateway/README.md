...
`VisiCore.StreamGateway` 与 Windows Device Worker、MediaMTX 部署在同一受控主机。它使用 Device Worker 令牌读取本机分配的录像机、摄像头路由和 DPAPI LocalMachine 密文，只在本机内存中解密设备凭据，并通过回环地址的 MediaMTX Control API 配置按需 RTSP 源。
