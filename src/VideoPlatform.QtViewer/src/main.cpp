...
int main(int argc, char *argv[]) {
    ...
    QApplication::setApplicationName(QStringLiteral("企业视频统一查看端"));
    QApplication::setOrganizationName(QStringLiteral("VideoPlatform"));
    CrashReporter::install();
    ThemeManager::instance().apply(application);

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("企业视频平台 Windows 原生查看客户端"));
    parser.addHelpOption();
    QCommandLineOption apiOption(QStringList{QStringLiteral("a"), QStringLiteral("api-url")},
                                 QStringLiteral("中心 API 基础地址"), QStringLiteral("url"), QStringLiteral("https://video-platform.local/"));
    ...
    if (parser.isSet(mpvLibraryOption)) {
        ...
        qputenv("VIDEO_PLATFORM_MPV_LIBRARY", mpvLibraryInfo.absoluteFilePath().toUtf8());
    }
    ...
    if (parser.isSet(verifyUiShellOption)) {
        QCoreApplication::setApplicationName(QStringLiteral("企业视频统一查看端-视觉验收"));
        ...
        verificationWindow.setWindowTitle(QStringLiteral("企业视频统一查看端 - 视觉验收"));
    ...
    if (parser.isSet(verifyLoginShellOption)) {
        QCoreApplication::setApplicationName(QStringLiteral("企业视频统一查看端-登录验收"));
        ApiClient verificationApiClient(QUrl(QStringLiteral("http://127.0.0.1:1/")), true);
