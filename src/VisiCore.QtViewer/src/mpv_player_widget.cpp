...
void writeMpvDiagnostic(const QString &message) {
    const QString path = qEnvironmentVariable("VISICORE_MPV_DIAGNOSTICS");
    if (path.isEmpty()) {
        return;
    }
    QFile file(path);
...
QStringList mpvRuntimeLibraryNames() {
    const QString configuredPath = qEnvironmentVariable("VISICORE_MPV_LIBRARY");
...
QString mpvRuntimeDisplayName() {
    return qEnvironmentVariableIsEmpty("VISICORE_MPV_LIBRARY")
