...
- `src/VideoPlatform.QtViewer` 是可选的 Windows 原生查看端源码，不包含在 Docker 安装中。
...
`v0.1.0` 只支持全新安装，不提供旧版内部数据库升级路径。后续发行版升级前先备份 `postgres-data` 卷；完成备份后停止服务、拉取或构建新镜像并重新启动。已有数据库不得再次运行 `setup`，该容器会检测到已有账号后拒绝修改数据。
...
```powershell
dotnet test .\tests\VideoPlatform.Api.Tests\VideoPlatform.Api.Tests.csproj --configuration Release
Push-Location .\src\VideoPlatform.Admin
npm ci
npm run typecheck
npm run build
Pop-Location
