...
适用 Linux x64 与 ARM64 主机。Docker 部署不在 `.env`、镜像、Compose 或 bootstrap 文件中填写中心地址、注册码或发行密钥。正式部署从 GitHub Release 的 `visicore-edge-agent-*-linux.tar.gz` 解压到固定目录；包内 Compose 已锁定发布镜像摘要，`.env` 只设置固定本机目录和配置页回环端口：

```bash
sudo install -d -m 0755 /opt/visicore/edge
sudo tar -xzf visicore-edge-agent-*-linux.tar.gz -C /opt/visicore/edge
sudo cp /opt/visicore/edge/.env.example /opt/visicore/edge/.env
cd /opt/visicore/edge
docker compose --env-file .env -f compose.yaml pull
docker compose --env-file .env -f compose.yaml up -d
```

首次部署还需由 root 执行一次 `/opt/visicore/edge/install-host-agent.sh`，它只安装受限 Host Agent、systemd 单元、固定 Compose 路径和 Unix Socket 权限，不写入环境专属配置。随后在宿主机浏览器访问 `http://127.0.0.1:18081`；远程维护通过 SSH 隧道访问该回环端口。

页面先测试中心 HTTPS 健康检查、Host Agent Socket、PEM 和域名白名单。确认配对后，配置服务仅通过受限 Unix Socket 提交固定配置对象；Host Agent 原子写入状态目录并执行固定的 `docker compose up --detach --no-deps edge-agent`。配置服务与 Edge Agent 均不挂载 Docker Socket。配对成功后 Agent 自动清除注册码。

Host Agent 默认部署但不允许实际升级。只有在页面中导入 PEM、公钥标识、受信下载域名并明确开启“允许实际受控升级”后，才会启用发行下载、验签和固定 Compose 升级。Host Agent 仍只接受目标为当前 Linux 架构、未过期、RSA-PSS SHA-256 验签成功且哈希匹配的发行清单。
