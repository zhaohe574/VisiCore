...
public sealed class ReleasePlanningTests
{
    ...
    private static string CreateDescriptorJson(string keyId)
    {
        ...
        return JsonSerializer.Serialize(new
        {
            ...
            artifacts = new[]
            {
                new { component = "core", platform = "linux", architecture = "amd64", artifactReference = $"ghcr.io/example/visicore-core@sha256:{digest}", artifactSha256 = digest, sizeBytes = 1L, minimumHostAgentVersion = "0.1.0" },
                new { component = "edge-docker", platform = "linux", architecture = "amd64", artifactReference = $"ghcr.io/example/visicore-edge-node@sha256:{digest}", artifactSha256 = digest, sizeBytes = 1L, minimumHostAgentVersion = "0.1.0" }
            }
        });
    }
}
