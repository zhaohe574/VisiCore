...
public sealed class StreamSessionPostgreSqlFixture : IAsyncLifetime
{
    ...
    {
        var configuredConnectionString = Environment.GetEnvironmentVariable("VISICORE_TEST_POSTGRES_CONNECTION");
        if (!string.IsNullOrWhiteSpace(configuredConnectionString))
        {
            ...
            {
                Database = $"visicore_integration_{Guid.NewGuid():N}"
            };
        ...
        container = new PostgreSqlBuilder("postgres:16-alpine")
            .WithDatabase("visicore_tests")
            .WithUsername("visicore")
    ...
    public async Task InitializeAsync()
    {
        ...
        dbContext.DevicePlugins.Add(new DevicePluginEntity
        {
            ...
            ManifestJson = JsonSerializer.Serialize(new DevicePluginManifest(
    ...
    private static string ValidateExternalConnectionString(string value)
    {
        ...
        if (string.IsNullOrWhiteSpace(builder.Database) ||
            !Regex.IsMatch(builder.Database, "^visicore_integration_[a-z0-9_]{1,32}$", RegexOptions.CultureInvariant) ||
            !IsLoopbackHost(builder.Host))
        {
            throw new InvalidOperationException(
                "VISICORE_TEST_POSTGRES_CONNECTION 仅允许指向回环地址、且数据库名以 visicore_integration_ 开头的临时数据库标识。");
        }
